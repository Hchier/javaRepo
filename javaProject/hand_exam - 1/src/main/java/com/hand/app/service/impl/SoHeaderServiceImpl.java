    ...
    public ResponseEntity<String> updateStatus(SoHeader soHeader, Principal principal, String status) {
        ...
        switch (status) {
            ...
            case APPROVED:
                ...
                if (!role.getName().equals("租户管理员")) {
                    //throw new RuntimeException("当前角色为：" + role.getName() + "。不为：sale_manager_201931073227");
                    System.out.println("当前角色为：" + role.getName() + "。不为：sale_manager_201931073227");
                    return Results.error("当前角色为：" + role.getName() + "。不为：sale_manager_201931073227");
                }
                if (!SUBMITED.equals(soHeader1.getOrderStatus())) {
                    //throw new RuntimeException("当前订单状态不是当前订单状态不是SUBMITED");
                    return Results.error("当前订单状态不是SUBMITED");
                }
            ...
            case REJECTED:
                ...
                if (!SUBMITED.equals(soHeader1.getOrderStatus())) {
                    //throw new RuntimeException("当前订单状态不是SUBMITED，无法更改为REJECTED");
                    return Results.error("当前订单状态不是SUBMITED，无法更改为REJECTED");
                }
                break;
            default:
                break;
        }
        soHeaderRepository.updateStatus(soHeader.getSoHeaderId(),status);
...
//
//        soHeader.setObjectVersionNumber(soHeader.getObjectVersionNumber()+1);
//
